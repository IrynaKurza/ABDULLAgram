    ...
    public static class Persistence
    {
        ...
        public static bool LoadAll(string path = AllPath)
        {
            ...
            try
            {
                ...
                foreach (var user in snap.Users) User.ReAdd(user);
                foreach (var img in snap.Images) Image.ReAdd(img);
                foreach (var txt in snap.Texts) Text.ReAdd(txt);
                foreach (var vid in snap.Videos) Video.ReAdd(vid);
                foreach (var f in snap.Files) ABDULLAgram.Attachments.File.ReAdd(f);
                foreach (var sticker in snap.Stickers) Sticker.ReAdd(sticker);
